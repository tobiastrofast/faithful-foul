...
var _SelectHotspot = class extends TypedEvent {
...
SelectHotspot.type = "select-hotspot";

// src/MapPlugin.ts
import { AbstractConfigurablePlugin, events, utils as utils3 } from "@photo-sphere-viewer/core";

// src/components/MapComponent.ts
import { AbstractComponent as AbstractComponent3, CONSTANTS as CONSTANTS2, SYSTEM as SYSTEM2, utils as utils2 } from "@photo-sphere-viewer/core";
  ...
  render() {
    ...
    [...this.config.hotspots, ...this.state.markers].forEach((hotspot) => {
      ...
      if (image) {
        ...
        context.beginPath();
        context.arc(0, 0, size * SYSTEM2.pixelRatio / 2, 0, 2 * Math.PI);
        context.fill();
      }
      context.restore();
    });
  ...
  },
  {
    overlayImage: (overlayImage, { rawConfig }) => {
      if (rawConfig.compassImage) {
        utils3.logWarn('map: "compassImage" is deprecated, use "overlayImage" instead');
