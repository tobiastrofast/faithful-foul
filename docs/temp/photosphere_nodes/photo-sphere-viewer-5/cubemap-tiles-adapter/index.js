  ...
  var CubemapTilesAdapter = class extends import_core3.AbstractAdapter {
    constructor(viewer, config) {
      ...
      if (this.viewer.config.requestHeaders) {
        ...
        this.loader = new import_three3.ImageLoader();
     ...
     */
    setOverlay() {
      throw new import_core3.PSVError("EquirectangularTilesAdapter does not support overlay");
    }
    ...
    __loadTile(tile, task) {
      return this.__loadImage(tile.url).then((image) => {
        if (!task.isCancelled()) {
          ...
          const material = new import_three3.MeshBasicMaterial({ map: import_core3.utils.createTexture(image) });
