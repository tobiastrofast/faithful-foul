...
"use strict";
  ...
  var _SelectHotspot = class extends import_core.TypedEvent {
    ...
    render() {
      ...
      [...this.config.hotspots, ...this.state.markers].forEach((hotspot) => {
        ...
        if (image) {
          ...
          context.beginPath();
          context.arc(0, 0, size * import_core6.SYSTEM.pixelRatio / 2, 0, 2 * Math.PI);
          context.fill();
        }
        context.restore();
      });
    ...
    },
    {
      overlayImage: (overlayImage, { rawConfig }) => {
        if (rawConfig.compassImage) {
          import_core7.utils.logWarn('map: "compassImage" is deprecated, use "overlayImage" instead');
